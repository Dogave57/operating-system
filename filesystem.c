...
int epic_findfile_inroot(unsigned int drive, char* filename, struct epic_file* pfilemd){
	...
	for (current_cluster = 1;current_cluster<last_cluster&&current_cluster<fshdr.last_cluster;current_cluster++){
...
int renamefile(struct file* pfile, char* newname){
	...
	if (pfile->fstype!=FS_EPIC)
		return -1;
	struct epic_fshdr fshdr = {0};
	if (epic_get_fsinfo(pfile->drive, &fshdr)!=0)
		return -1;
	struct epic_fshdr* pfshdr = &fshdr;
...
int getfilelist(unsigned int drive, struct file* pdir, struct fileinfo** pplist, unsigned int* plist_entries){
	...
	unsigned int current_cluster = filemd.data;
	if(!pdir)
		current_cluster = 1;
